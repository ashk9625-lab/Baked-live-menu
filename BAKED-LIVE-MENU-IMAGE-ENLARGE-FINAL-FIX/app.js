...
(function(){
  ...
  function ensureProductImageViewer(){
    ...
    style.id='bakedProductImageViewerStyles';
    style.textContent=`
      #productImageViewer{position:fixed;inset:0;z-index:2147483647;display:none;align-items:center;justify-content:center;padding:20px;box-sizing:border-box}
      #productImageViewer.open{display:flex!important}
      #productImageViewer .biv-backdrop{position:absolute;inset:0;background:rgba(0,0,0,.9)}
      #productImageViewer .biv-dialog{position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;justify-content:center;max-width:96vw;max-height:94vh}
      #productImageViewer .biv-image{display:block;width:auto;height:auto;max-width:94vw;max-height:84vh;object-fit:contain;background:#fff;border-radius:16px;box-shadow:0 20px 70px rgba(0,0,0,.45)}
      #productImageViewer .biv-close{position:absolute;top:-16px;right:-16px;z-index:3;width:46px;height:46px;border:0;border-radius:50%;background:#fff;color:#111;font-size:32px;line-height:1;cursor:pointer;box-shadow:0 5px 20px rgba(0,0,0,.35)}
      #productImageViewer .biv-caption{margin-top:12px;color:#fff;font-size:16px;font-weight:800;text-align:center}
      .product-image img,.featured-card .product-image img,.surprise-image img{cursor:zoom-in!important}
      @media(max-width:600px){#productImageViewer{padding:10px}#productImageViewer .biv-image{max-width:96vw;max-height:80vh;border-radius:12px}#productImageViewer .biv-close{top:8px;right:8px}}
    `;
    ...
    modal.setAttribute('aria-hidden','true');
    modal.innerHTML=`
      <div class="biv-backdrop" data-biv-close></div>
      <div class="biv-dialog" role="dialog" aria-modal="true" aria-label="Product image viewer">
        <button type="button" class="biv-close" data-biv-close aria-label="Close image">×</button>
        <img class="biv-image" alt="Product image">
        <div class="biv-caption"></div>
      </div>`;
    document.body.appendChild(modal);
    modal.querySelectorAll('[data-biv-close]').forEach(el=>el.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();closeProductImageViewer();}));
    return modal;
  }
  ...
  function openProductImageViewer(img){
    ...
    const full=modal.querySelector('.biv-image');
    const caption=modal.querySelector('.biv-caption');
    full.src=src;
    full.alt=img.dataset?.productName||img.alt||'Product image';
    caption.textContent=img.dataset?.productName||img.alt||'';
  ...
  document.addEventListener('click',function(e){
    const img=e.target.closest('.product-card .product-image img, .featured-card .product-image img, .surprise-image img');
    if(!img)return;
    e.preventDefault();
    e.stopPropagation();
