...
function closeProductImageViewer(){const v=$('#productImageViewer');if(v){v.classList.add('hidden');$('#productImageViewerImg').src='';document.body.classList.remove('image-viewer-open')}}
document.addEventListener('click',e=>{
 const im=e.target.closest('.product-image img');
 if(im){e.preventDefault();e.stopPropagation();const v=$('#productImageViewer');$('#productImageViewerImg').src=im.currentSrc||im.src;$('#productImageViewerName').textContent=im.closest('.product-card')?.querySelector('h3')?.textContent||im.alt||'';v.classList.remove('hidden');document.body.classList.add('image-viewer-open');return}
 if(e.target.id==='productImageViewer'||e.target.closest('#productImageViewerClose'))closeProductImageViewer()
});
document.addEventListener('keydown',e=>{if(e.key==='Escape')closeProductImageViewer()});
