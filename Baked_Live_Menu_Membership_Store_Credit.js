...
function openMembershipCard() {
  ...
  if (!member) {
    ...
    const creditBtn=document.getElementById('memberCredit');
    if(creditBtn) creditBtn.onclick=()=>manageStoreCredit(member);
  }
...
function renderMembership(member) {
  return `
    <div class="member-card">
      ...
      <div class="member-number">${escapeHtml(member.memberNumber)}</div><div style="margin-top:12px;font-size:13px;opacity:.72">STORE CREDIT</div><div style="font-size:25px;font-weight:900">${money(Number(member.storeCredit||0))}</div>
    ...
    <div class="member-actions">
      ...
      <button class="member-secondary" id="memberEdit" type="button">Edit details</button></div><div class="member-actions"><button class="member-secondary" id="memberCredit" type="button">Manage store credit</button>
    </div>
...
function manageStoreCredit(member) {
  if (!accessToken) return toast('Admin login is required to manage store credit');
  const raw = prompt(`Current store credit: ${money(Number(member.storeCredit||0))}\n\nEnter amount to ADD. Use a negative amount to deduct:`, '0');
  if (raw === null) return;
  const amount = Number(String(raw).replace(/[^0-9.-]/g,''));
  if (!Number.isFinite(amount) || amount === 0) return toast('Enter a valid credit amount');
  member.storeCredit = Math.max(0, Number(member.storeCredit||0) + amount);
  saveMember(member);
  toast(`Store credit updated to ${money(member.storeCredit)}`);
  openMembershipCard();
}
